...
func TestHandleVercelStreamPrepareAppliesCurrentInputFile(t *testing.T) {
	...
	if !strings.Contains(promptText, promptcompat.GetCurrentVariant().HistoryFilename) {
...
func TestHandleVercelStreamPrepareUploadsToolsSeparately(t *testing.T) {
	...
	payloadPrompt, _ := payload["prompt"].(string)
	for label, promptText := range map[string]string{"final_prompt": finalPrompt, "payload.prompt": payloadPrompt} {
		if !strings.Contains(promptText, promptcompat.CurrentToolsContextFilename()) || !strings.Contains(promptText, "TOOL CALL FORMAT") {
...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	stdReq := promptcompat.StandardRequest{
		...
		FinalPrompt:             fmt.Sprintf(promptcompat.GetCurrentVariant().InlinePromptText, promptcompat.GetCurrentVariant().HistoryFilename) + " " + promptcompat.CurrentToolsContextFilename(),
	...
	promptText, _ := payload["prompt"].(string)
	if !strings.Contains(promptText, promptcompat.CurrentToolsContextFilename()) {
