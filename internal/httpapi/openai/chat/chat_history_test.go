...
func TestChatCompletionsCurrentInputFilePersistsNeutralPrompt(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != promptcompat.CurrentInputContextFilename() {
		t.Fatalf("expected history upload, got %q", ds.uploadCalls[0].Filename)
	}
	...
	if !strings.Contains(full.Messages[0].Content, promptcompat.GetCurrentVariant().HistoryFilename) {
