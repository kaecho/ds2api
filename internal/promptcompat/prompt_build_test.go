...
func TestBuildOpenAIPromptWithToolInstructionsOnlyOmitsSchemas(t *testing.T) {
	...
	if !strings.Contains(finalPrompt, "Treat "+CurrentToolsContextFilename()+" as the authoritative list of callable tools and schemas") {
...
func TestBuildOpenAIToolsContextTranscriptContainsOnlyDescriptions(t *testing.T) {
	...
	for _, want := range []string{toolsTranscriptTitle(), "You have access to these tools", "Tool: search", "Description: search docs", `Parameters: {"type":"object"}`} {
