...
- 公开模型名到线协议 `model_type` 的映射仍是 flash→`default`、pro→`expert`、vision→`vision`。官方 App 2.5.1 首页仍不选档，completion 仍带这个字段。同一 `chat_session_id` 以**第一条 completion** 的 `model_type` 绑定，后续覆盖无效。细节见 [Android 2.5.1 客户端逆向](./DeepSeekAndroid客户端逆向-2026-09-14.md)。
